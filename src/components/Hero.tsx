          ...
          {/* Urgency pill + location pill */}
            ...
            <div className="grid grid-cols-2 gap-2 animate-fadeInUp" style={{ animationDelay: '0.28s' }}>
              {TRUST_CHIPS.map(({ icon: Icon, label }, i) => (
                <div
                  key={label}
                  className="flex items-center gap-2.5 bg-black/50 border border-white/15 rounded-xl px-3.5 py-3 animate-badgePop backdrop-blur-sm"
                  style={{ animationDelay: `${0.32 + i * 0.07}s` }}
                >
                  <div className="w-7 h-7 flex-shrink-0 bg-primary/20 border border-primary/30 rounded-lg flex items-center justify-center">
